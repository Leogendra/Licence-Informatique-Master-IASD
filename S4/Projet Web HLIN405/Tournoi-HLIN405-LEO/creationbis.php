<?php 
  ...
  $Nom_Tournoi=$_POST["Nom_Tournoi".$tour];
  $Categorie=$_POST["Categorie"];
  $Lieu=$_POST["Lieu"];
  $Date_Debut=$_POST["Date_Debut"];
  $Date_Fin=$_POST["Date_Fin"];
  $NbTournois=$_POST["NbTournois"];
  $Nombre_Equipes_Tournoi=$_POST["Nombre_Equipes_Tournoi".$tour];
  $Description=$_POST["Description"];
  $Plus_Infos=$_POST["Plus_Infos".$tour];

    if ($Date_Debut == $Date_Fin) { $ok='required'; $message='<i class="bi bi-hourglass-split"></i> En cours...'; }
...
<html>
  ...
  <style>
    body {position: relative; background-color: rgb(0, 0, 0); margin-bottom:5em}
    h1 {color: white;}
          ...
          <div class="card text-white bg-dark border-primary">
            ...
            <div class="card-body">
              ...
              <form class="row" method="post" action="">
                  ...
                  <div class="col-md-8 col-xl-8 my-1">
                    <label class="form-label">Nom de l'événement</label>
                    <input type="text" class="form-control bg-dark text-white <?=$valid?>" name="Nom_Evenement" value="<?php echo $Nom_Evenement?>" placeholder="Roland-Garros 2021" <?=$ok?>>
                  </div>
                ...
                ?>
                  ...
                  <div class="col-md-12 my-1">
                    ...
                    <input type="text" class="form-control border-primary bg-dark text-dark"  placeholder="Tournoi <?=$tour?>" disabled>
                  </div>

                  <div class="col-md-8 my-1">
                    <label class="form-label">Nom du tournoi</label>
                    <input type="text" class="form-control bg-dark text-white <?=$valid?>" <?='name="Nom_Tournoi'.$tour.'"'?> value="<?php echo $Nom_Tournoi?>" placeholder="Roland-Garros 2021" <?=$ok?>>
                  </div>

                  <div class="col-md-4 my-1">
                    <label class="form-label">Nombre d'équipes</label>
                    <select class="form-select bg-dark text-white <?=$valid?>" value="<?php echo $Nombre_Equipes_Tournoi?>" <?='name="Nombre_Equipes_Tournoi'.$tour.'"'?> <?=$ok?>>
...
$req->execute(array($_POST['Nom_Evenement'], $_POST['Description'],$_POST['Lieu'], $_POST['Date_Debut'],$_POST['Date_Fin'], $_POST['Categorie'],$_SESSION['login']));
...
$req->execute(array($result['Id_Evenement'], $_POST['Nom_Tournoi'.$tour], $_POST['Nombre_Equipes_Tournoi'.$tour], $_POST['Plus_Infos'.$tour]));
$tour -= 1;
}
        ...
        <div class="col">
          <div class="card text-white bg-dark border-primary">
            ...
            <div class="card-body">
              ...
              <p class="card-text">Vous pouvez aussi créer un tournoi</p> 
            </div>
